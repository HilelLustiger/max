...
def create_habit(
    ...
    habit = Habit(title=title, goal_id=goal_id, description=description, frequency=frequency)
    session.add(habit)
    session.flush()
    return habit
...
def list_habits(
    session: Session, goal_id: str | None = None, status: HabitStatus | None = HabitStatus.ACTIVE
) -> list[Habit]:
