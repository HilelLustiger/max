...
def load_turn(session: Session, channel: str, external_id: str, text: str, request_id: str) -> TurnContext:
    ...
    conversation_id as thread_id, see ADR-0007) supplies it automatically. Whether this turn
    resumes a pending clarification is also the checkpointer's call, not ours (see ADR-0008)."""
...
def persist_turn_result(
    ...
    """Persist the graph's result. A resumed turn (tool invoked deterministically, no LLM call
    for the reply itself) is told apart from a normal LLM turn by `resumed` in the reply's
    response_metadata, set by the graph itself."""
...
def persist_turn_clarification(
    session: Session, ctx: TurnContext, request_id: str, tool_call_message: AIMessage, question: str
) -> None:
    """Persist a turn that paused on interrupt() to ask a clarification (see ADR-0008).
