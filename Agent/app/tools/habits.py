...
def create_habit(title: str, frequency: str = "daily") -> str:
    ...
    with get_session() as session:
        ...
        return f"Created habit '{habit.title}' (id={habit.id}, frequency={habit.frequency})."
...
def list_habits(status: str | None = None) -> str:
    ...
    return "\n".join(
        f"- {habit.title} (id={habit.id}, frequency={habit.frequency})" for habit in habits
    )
...
def log_habit(habit_id: str, notes: str | None = None) -> str:
    ...
    with get_session() as session:
        ...
        return f"Logged '{habit.title}' at {log.completed_at.isoformat()}."
